...
resource "null_resource" "docker_build_push" {
  ...
  provisioner "local-exec" {
    command = <<-EOF
      ...
      # docker build --no-cache --platform linux/amd64,linux/arm64 --build-arg PYTHON_VERSION=3.14 -t ${var.application_name} -f ${var.dockerfile_path} --progress=plain ${var.docker_build_context}
      docker build --no-cache --platform linux/amd64 --build-arg PYTHON_VERSION=3.14 -t ${var.application_name} -f ${var.dockerfile_path} --progress=plain ${var.docker_build_context}
