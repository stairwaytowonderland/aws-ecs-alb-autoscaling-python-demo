...
ids() {
  terraform -chdir="${SCRIPT_DIR}/../infra/aws/app" output -json ephemeral_instance_ids 2>/dev/null | jq '.[]' --raw-output
}
...
main() {
  # shellcheck disable=SC2046  # We want word splitting here to pass multiple instance IDs to describe_ec2
